# P6 Phase4：Rightmove 抓取 → 标准化 → storage 调试入口（在 rental_app 下执行）
#   python scripts/run_rightmove_pipeline.py --limit 3
...
from data.pipeline.rightmove_pipeline import run_rightmove_pipeline  # noqa: E402
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(
        description="Rightmove → normalizer → storage 闭环（P6 Phase4）",
    )
    ...
    p.add_argument("--save-raw", action="store_true", help="写入 rightmove_raw_sample.json")
    p.add_argument(
        "--save-normalized",
        action="store_true",
        help="写入 rightmove_normalized_sample.json",
    )
