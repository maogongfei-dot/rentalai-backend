...
- **读取**：**`GET /api/analysis/history/records`** → `{ success, message, records }`。前端仍以 **localStorage 统一历史** 为主；辅助脚本 **`web_public/assets/server_history_api.js`**（**`?server_history=1`** 探测）。
...
- 产品：前端**全面**改读服务端历史、与本地合并策略；**数据库**迁移。
