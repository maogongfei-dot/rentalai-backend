# P7 Phase2：Zoopla 抓取 → normalizer → storage 闭环（单页；镜像 rightmove_pipeline）
...
from data.scraper.zoopla_scraper import ZooplaScraper
...
_RAW_SAMPLE_PATH = _DEBUG_DIR / "zoopla_raw_sample.json"
_NORMALIZED_SAMPLE_PATH = _DEBUG_DIR / "zoopla_normalized_sample.json"


class ZooplaPipelineResult(TypedDict, total=False):
    success: bool
    error: str | None
    raw_count: int
...
def _maybe_attach_normalized_listings(
    out: ZooplaPipelineResult,
    normalized: list[ListingSchema] | None,
    include: bool,
) -> ZooplaPipelineResult:
    if not include:
        return out
...
def run_zoopla_pipeline(
    *,
    ...
    """
    ZooplaScraper → normalize_listing_batch(source=zoopla) → save_listings。

    - `normalization_skipped`：`is_valid_listing_payload` 未通过的核心弱记录数。
    - `skipped`：storage 单条失败计数（与 `save_listings` 一致）。
    ...
    try:
        raw_rows = ZooplaScraper().scrape(query=scrape_q, limit=limit)
    ...
    try:
        normalized = normalize_listing_batch(stamped, source="zoopla")
...
def scrape_and_normalize_zoopla(**kwargs: Any) -> ZooplaPipelineResult:
    """`run_zoopla_pipeline` 的别名。"""
    return run_zoopla_pipeline(**kwargs)


def run_zoopla_normalization_pipeline(**kwargs: Any) -> ZooplaPipelineResult:
    """`run_zoopla_pipeline` 的别名。"""
    return run_zoopla_pipeline(**kwargs)
