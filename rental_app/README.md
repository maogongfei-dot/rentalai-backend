...
**一页最短流程**（依赖、命令、脚本、访问 URL）：**[`LOCAL_RUN.md`](LOCAL_RUN.md)**（Phase 6 第 3 步）。
...
### Phase 6 — 部署可行性检查（第 1 步）

- **上线前结构梳理与检查清单**：**`DEPLOY_READINESS.md`**（与 **`P10_PHASE5_DEPLOYMENT.md`** 互补：偏「是否具备最小可部署条件」）。
- **Python**：**`requirements.txt`** + **`runtime.txt`**；**`run.py`** 为推荐入口。
- **前端静态**：主路径**无独立 `npm dev`**；**`web_public/package.json`** 仅用于 **`npm run build`**（可选 API base 注入）。详见 **`DEPLOY_READINESS.md`**。

### Phase 6 — 环境配置可切换（第 2 步）

- **后端**：**`config.py`** — `RENTALAI_ENV`、`PORT`/`RENTALAI_PORT`、`RENTALAI_DEBUG`、`RENTALAI_RELOAD` 等；Streamlit 默认 API 根由 **`get_default_api_url_for_tools()`**（`RENTALAI_API_URL` 或 `RENTALAI_PUBLIC_API_HOST` + 端口）。
- **前端**：**`web_public/assets/api_config.js`** — 从页面 **meta** 读 **`RENTALAI_API_BASE`** 分域根；**`scripts/inject-api-base.mjs`** 在构建时写入 meta（含可选 **`RENTALAI_ENV`**）。

### Phase 6 — 本地启动与脚本（第 3 步）

- **最短说明**：**[`LOCAL_RUN.md`](LOCAL_RUN.md)**（依赖、启动、访问、环境变量、可选冒烟）。
- **启动**：**`python run.py`**；Windows 可用 **`start_local.bat`** 或 **`scripts/run_backend.bat`**；Unix：**`bash scripts/run_backend.sh`**。

---

## 说明

- 请勿提交含密钥的 **`.env`**；模板为 **`.env.example`**；本地最短步骤见 **`LOCAL_RUN.md`**。
