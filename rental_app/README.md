...
### 智能入口（Phase 4 第七轮 · Step1）

- **路由**：**`/assistant`**（`web_public/assistant.html`），顶栏与首页主功能区可进入。
- **当前能力**：多行自然语言描述 + **「保存草稿并继续」** → 写入 **`sessionStorage`** 键 **`rentalai_assistant_draft`**（本会话）；**不**解析意图、**不**自动跳转分析 API。
- **下一步**：意图分类（房源 vs 合同）或把草稿**预填**到 `#ai-query` / 合同粘贴框。
