...
- **互通**：顶栏与各页链接可在两条主流程间切换（`web_public/assets/auth_local.js`）；顶栏 **「分析历史」** 指向 **`/analysis-history`**（与首页次级入口一致）。

**尚未落地的产品增强**（可后续迭代）：分析历史页**交互深化**（删除/清空、点进详情）、聊天式统一输入、真实用户系统与后端鉴权。统一入口 **`/analysis-history`**（Phase 4 第六轮）已提供；**最近分析摘要**存 **`localStorage` 键 `rentalai_unified_analysis_history_v1`**（房源/合同成功分析后自动追加，含 **`detail_snapshot`** 供 `/analysis-history` 展开回看；与手动「保存」的 `analysis_history` 全量列表并存）。
