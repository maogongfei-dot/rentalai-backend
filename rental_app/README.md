...
本章用于统一说明 `rental_app/` 内关键路径与仓库其余部分的角色，降低「入口混用、改错层级」的风险。下文与后文「项目边界」「启动方式」等章节一致互补，**不替代**那些约束性说明。

### A. 主线文件（Primary Path）

| 路径 | 角色 |
|------|------|
| **`run.py`** | **当前唯一推荐的本地启动入口**：一键拉起主产品常用进程（开发与本地验证请优先使用）。 |
| **`api_server.py`** | **当前主后端入口**：FastAPI 应用定义处，核心 HTTP API、静态页挂载、路由聚合等均以此为中枢。 |
| **`web_public/`** | **当前主前端静态页面目录**：与 FastAPI 一并服务或可被独立部署（如 Vercel）的前端资源根；静态 HTML/CSS/JS 主交付面在此。 |

### B. 部署层（Deployment Layer）

| 路径 | 角色 |
|------|------|
| **`app.py`** | **Deployment shim（部署导入入口）**：供部分托管/导入场景指向 ASGI 应用，**不是**日常本地主开发入口；日常本地请用 `run.py`。 |

### C. 辅助 / 旧 UI 层（Legacy / Auxiliary UI）

| 路径 | 角色 |
|------|------|
| **`app_web.py`** | **旧的 / 辅助的 Streamlit UI**：历史与内测演示可用，**不是**当前主产品入口；主产品交互与集成以 FastAPI + `web_public/` 为准。 |

### D. 历史层 / 非主线文件（Historical / Non-primary Layer）

仓库**根目录**下的 `main.py`、`app.py`、`modules`、`backend` 等，在本文档中**不作为当前主开发主线**看待。它们可能承载历史实验、旧脚手架或兼容用途，**默认视为历史层、旧实验层或兼容层**；后续常规功能迭代与排障，**默认不以这些路径为优先改动对象**，除非有明确兼容或迁移任务。

> 说明：`rental_app/app.py`（上文 B）与根目录同名文件若同时存在，角色不同——**主产品约定以 `rental_app/` 内 README 与入口表为准。**

### 业务结构映射

- **RentAI**：长租**主系统**，平台能力闭环的核心承载。
- **ShortRentAI**：平台内的**短租扩展板块**，与 RentAI **并列扩展**，**不替代** RentAI。
- **ShortRentAI 当前重点数据方向**：包括 **SpareRoom** 等数据源方向（随路线图演进，以代码与产品说明为准）。
- **ShortRentAI 后续能力**：将加入**房东自发布**能力；发布侧将支持**图片 / 视频 / 2D / 3D / VR** 等素材形态（分期落地）。
- **信任与人工核实系统**：设计为**跨 RentAI 与 ShortRentAI 共用**的平台能力，而非单属于某一子板块。

### 后续开发默认规则

1. **默认只优先修改 `rental_app/` 内的主线文件**（见本章 A、B；静态面见 `web_public/`）。
2. **新功能优先接入当前 FastAPI Web 主产品**（`api_server.py` + `web_public/`，由 `run.py` 统一拉起验证）。
3. **默认不再把 Streamlit（`app_web.py`）作为默认主入口**；仅在明确需要旧界面或内部演示时启用。
4. **默认不再把仓库根目录旧文件当成当前主产品入口**；避免在新需求上从根目录脚手架「顺手扩展」导致双主线。

---

## 当前主产品用户流程（Primary User Flow）

本章描述**默认主流程**，作为后续需求评审、接口设计与页面改动的对齐基准；与「当前产品地图」中的技术入口互为补充。

### 默认主流程（端到端）

1. **进入**：用户打开 **`/`（首页）** 或 **`/assistant`（智能入口）**，作为当前主产品的默认入口面。
2. **输入**：用户在页面上**用自然语言描述租房需求**或提出与找房相关的问题（含后续迭代中的多轮补充）。
3. **主分析**：系统进入 **RentAI** 主流程——对需求结构化解析，并串联推荐、评分、Explain、决策与风险提示等核心能力（以 FastAPI + `web_public` 为载体）。
4. **合同分支**：若用户表述或显式操作指向**合同 / 租约条款**（例如进入合同分析页或使用合同相关能力），则进入**合同分析分支**，与「纯找房」路径并行存在，共享同一后端体系下的专用路由与页面。
5. **结果呈现**：系统向用户返回**分析结论、分项解释、行动建议与风险提示**（展示形态随版本迭代可在接口契约内扩展，但语义层次保持稳定）。
6. **历史沉淀**：在适用场景下将本次运行**写入历史记录**（访客以本地策略为准，登录用户可同步服务端记录，以实现「可回看」）。
7. **回访**：**登录用户**可通过 **历史页、分析历史聚合页、账户页** 等入口查看、管理或延续此前的分析记录。

### RentAI、ShortRentAI 与跨域能力

- **RentAI** 是平台的**主系统**（长租决策与找房主路径），主流程优先级与资源聚焦首先落在 RentAI 闭环。
- **ShortRentAI** 是平台内的**短租扩展板块**，在架构上为「主系统 + 子板块」的**并存关系**，**不替代** RentAI。
- 当用户需求落在**短租、合租、灵活租期**等范畴时，可路由或引导至 **ShortRentAI 分支**（随路线图在数据与 UI 上逐步贯通）。
- **ShortRentAI** 当前与后续的重点方向包括：**SpareRoom 等数据源**、**房东自发布**、以及发布侧的**媒体素材能力**（**图片 / 视频 / 2D / 3D / VR** 等，分期落地）。
- **信任与人工核实系统**在设计上**同时作用于 RentAI 与 ShortRentAI**，作为跨子板块的统一信任层，而非重复建设两套逻辑。

### 当前默认开发规则（与用户流程对齐）

1. **后续新功能优先接入上述主产品用户流程**（首页 / 助理入口 → RentAI 主链 → 结果与历史），在既有页面与 API 契约上**迭代增强**，而非另起并列故事线。
2. **不优先单独开新入口**（例如隐藏 URL、仅脚本可调、与主流程无导航关系的试验页），除非经评审确认为阶段性实验并明示下线策略。
3. **不再将旧的 Streamlit UI（`app_web.py`）视为默认产品主线**；辅助演示或兼容场景除外。
4. **RentAI 与 ShortRentAI** 采用 **「主系统 + 子板块」** 结构长期共存：新能力先判断是否归属 RentAI 主链，再评估 ShortRentAI 扩展点，避免两套产品定义互相替代。
