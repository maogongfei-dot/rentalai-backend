...
**当前用户体系（Phase 5 第二轮）**：**最小前端会话**（`localStorage`：Bearer Token + 用户 id/邮箱，或本地演示 `current_user`）。可选调用后端 `/auth/login`、`/auth/register`（SQLite 用户表），但**产品侧仍以本机状态与分桶为主**，**不等同**生产级账号中心。收藏与历史按 **`userId` / `guest` 分桶**存本机。详见下文「Phase 5 第二轮」。
...
| 账户与登录 | `/login`、`/register`；顶栏 **Logout**；**`/account`** 查看会话与分桶；未登录访问受保护页会跳 `/login` |
