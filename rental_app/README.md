...
| **历史读取** | **`GET /api/analysis/history/records?userId=…&type=property|contract`** → `{ success, message, records }`；见 `assets/server_history_api.js`；**Phase 5 第四轮**起已登录用户在前端**优先**消费此接口（见下「双模式历史」） |

**仍为 Demo 级**：进程内 Bearer 表、**无** HttpOnly Cookie、**无** 按请求强制鉴权、历史读接口**不**校验 token（任意知道 `userId` 的客户端可拉取该桶——需后续**受保护 API**）。

**刻意未在第三轮落地**：PostgreSQL/SQLite 迁移、**bcrypt/Argon2**、**受保护 API**。第四轮已做前端**双模式读写**；**未**做 guest→user 自动迁移合并、分页与冲突处理（见「Phase 5 第四轮」）。
...
**当前限制**：历史读接口仍**公开 query**；无分页/搜索；无多设备实时同步；登录前后 **guest 与 user 桶不自动合并**；云端失败时仅回退本机，无重试队列。

**本轮刻意未做**：真正 **token 绑定 + 受保护历史 API**；guest 记录**一键迁移**到账户；**分页 / 搜索 / 筛选**；**冲突解决**与后台合并；**数据库**由 JSON 升级为业务库。
...
**本步未做**：受保护历史接口、HttpOnly、多 worker 共享会话。
