...
**仍为 Demo 级**：进程内 Bearer 表、**无** HttpOnly Cookie；**历史读取**已需 Bearer（Step3），**历史写入**仍主要依赖请求体 `userId`（未全面改为仅 token）。
...
| **未做** | **JWT**、过期与 **refresh**、**全接口**强制鉴权、**history 写入**与 Bearer 绑定、**bcrypt/Argon2**、**DB** 升级 |
| **推荐下一步（择一）** | **保护 history 写入**（body `userId` 与 token 一致）→ 或 **密码 hash / JWT**（安全加深）→ 或 **继续产品功能**（分页、迁移 UX）；**数据库升级**在需要持久化与多实例时再做 |
